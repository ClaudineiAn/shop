...
let prevActive = null
const books = ref([])
const actualCat = null
const pathnameSplited = window.location.pathname.match(/[^V]+$/)[0]
...
export const dataBooks = () => {
  return {
    ...
    first: true,
    actualCat: null,
    pathnameSplited: window.location.pathname.match(/[^V]+$/)[0],
    extIndex: -1,
    playing: true,
    counter: ref(10),
    intervalId: null,
...
async function generateBooksHTML() {
  let html = ''
  let extIndex = 0
  let first = true
  let booksNav = ''
