...
const setusernameError = (v, setError) => {
  setError(v);
};
...
const getAccounts = async (web3) => {
  ...
  if (!accounts || accounts.length === 0) {
    throw new Error('No accounts found. Please login to MetaMask.');
  }
  return accounts;
};

const handleAccountRegistration = async (userAuthContract, username, account, router, setError) => {
  const registeredUsername = await userAuthContract.methods.getUser().call({ from: account });

  if (!registeredUsername) {
    if (confirm('You are about to create a new account. Is this what you would like?')) {
      await userAuthContract.methods.register(username).send({ from: account });
...
export const validation = async (router, username, setError) => {
  ...
  try {
    ...
    await handleAccountRegistration(userAuthContract, username, account, router, setError);
  } catch (error) {
